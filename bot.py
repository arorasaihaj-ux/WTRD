...
TIER_NAMES = {'trial': 'Beginner Middleman', 'middleman': 'Advanced Middleman', 'pro': 'Middleman', 'head': 'Head Middleman', 'owner': 'Owner'}
TIER_LIMITS = {'trial': 'Trades up to 2k Robux (No Fee)', 'middleman': 'Trades up to 6k Robux (No Fee)', 'pro': 'Trades up to 10k Robux (No Fee)', 'head': 'Trades up to 20k Robux (No Fee)', 'owner': 'Trades 20k+ Robux (Fee: 100 Robux or 20M Brainrot)'}
...
class TierSelectView(discord.ui.View):
    ...
    @discord.ui.select(placeholder="Select middleman tier based on your trade value", options=[discord.SelectOption(label="Beginner Middleman", value="trial", description="Up to 2k Robux (No Fee)", emoji="🌱"), discord.SelectOption(label="Advanced Middleman", value="middleman", description="Up to 6k Robux (No Fee)", emoji="💼"), discord.SelectOption(label="Middleman", value="pro", description="Up to 10k Robux (No Fee)", emoji="⚡"), discord.SelectOption(label="Head Middleman", value="head", description="Up to 20k Robux (No Fee)", emoji="👑"), discord.SelectOption(label="Owner", value="owner", description="20k+ Robux (Fee: 100 Robux/20M Brainrot)", emoji="💎")])
...
class TradeDetailsModal(discord.ui.Modal, title="Fill out the Format"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            review_embed = discord.Embed(description="⭐ Vouching and rating the middleman after trade is strictly necessary\nCopy the middleman user ID and paste it in 'submit a review'", color=discord.Color.gold())
...
async def botinfo(interaction: discord.Interaction):
    embed = discord.Embed(title="🤖 Middleman Bot Information", description="Professional middleman service bot for secure trading", color=discord.Color.blue(), timestamp=datetime.utcnow())
...
async def remove_user(interaction: discord.Interaction, user: discord.Member):
    ...
    if ticket['claimed_by'] == interaction.user.id:
        ...
        has_permission = True
    elif has_middleman_role(interaction.
